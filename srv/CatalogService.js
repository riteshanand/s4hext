...
module.exports = cds.service.impl(async function (srv) {
    ...
    var getAllSalesOrders = async function () {
        const { salesOrderService } = require('./src/generated/OP_API_SALES_ORDER_SRV_0001');
        const { salesOrderApi } = salesOrderService();
        const dataSalesData = await salesOrderApi.requestBuilder().getAll().top(30).select(
    ...
    srv.on('READ', MySalesOrder, async function (req) {
